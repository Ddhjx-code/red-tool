...
(function () {
  ...
  var C = D.C;
  var ARROW_TYPES = D.ARROW_TYPES;
  var ENEMY_TYPES = D.ENEMY_TYPES;
  var WALL_TYPES = D.WALL_TYPES;
  ...
  function makeJinwu(x, y, opt) {
    opt = opt || {};
    var type = opt.type || 'jinwu';
    var et = ENEMY_TYPES[type] || ENEMY_TYPES.jinwu;
    var R = et.radius;
    var scaleK = et.scale / BIRD_SCALE;
    var b = M.Bodies.circle(0, 0, R * BIRD_SCALE, {
      friction: 0.75, frictionStatic: 0.9, restitution: 0.04,
      density: 0.0010, slop: 0.02
    });
    b.plugin.hp = et.hp;
    b.plugin.maxHp = et.hp;
    b.plugin.label = et.name;
    b.plugin.kind = 'jinwu';
    b.plugin.enemyType = type;
    b.plugin.roles = PART_ROLES['jinwu'];
    b.plugin.shapeName = 'jinwu';
    b.plugin.score = et.score;
    if (et.behavior === 'patrol') {
      b.plugin.patrolCenter = opt.patrolCenter || x;
      b.plugin.patrolRange = opt.patrolRange || et.patrolRange || 80;
      b.plugin.patrolSpeed = et.patrolSpeed || 1.5;
      b.plugin.patrolPhase = 0;
    }
    if (et.deathExplosion) {
      b.plugin.deathExplosion = et.deathExplosion;
    }
    var allX = [], allY = [];
    Object.keys(D.SHAPES.jinwu).forEach(function (n) {
      D.SHAPES.jinwu[n].forEach(function (p) { allX.push(p[0]); allY.push(p[1]); });
    });
    b.plugin.visBox = {
      dx: ((Math.min.apply(null, allX) + Math.max.apply(null, allX)) / 2) * BIRD_SCALE,
      dy: ((Math.min.apply(null, allY) + Math.max.apply(null, allY)) / 2) * BIRD_SCALE,
      w: (Math.max.apply(null, allX) - Math.min.apply(null, allX)) * BIRD_SCALE,
      h: (Math.max.apply(null, allY) - Math.min.apply(null, allY)) * BIRD_SCALE
    };
  ...
  function makeFusan(shapeName, x, y, wallType) {
    wallType = wallType || 'fusan';
    var wt = WALL_TYPES[wallType] || WALL_TYPES.fusan;
    var b = compoundFrom(shapeName, {
      friction: wt.friction, frictionStatic: wt.frictionStatic || 1.0,
      restitution: wt.restitution, density: wt.density, slop: 0.02
    }, 1);
    b.plugin.hp = wt.hp;
    b.plugin.maxHp = wt.hp;
    b.plugin.label = wt.name;
    b.plugin.wallType = wallType;
    b.plugin.fireResist = !!wt.fireResist;
    if (wt.shardOnBreak) b.plugin.shardOnBreak = true;
    if (wt.arrowSlow) b.plugin.arrowSlow = wt.arrowSlow;
    M.Body.setPosition(b, { x: x, y: y });
    M.Body.setAngle(b, 0);
    return b;
  }
  ...
  function makeArrow(arrowType) {
    arrowType = arrowType || 'plain';
    var at = ARROW_TYPES[arrowType] || ARROW_TYPES.plain;
    var b = compoundFrom('arrow', {
      friction: 0.3, restitution: 0.22, density: at.density,
      frictionAir: 0, slop: 0.01
    }, 1);
    b.frictionAir = at.frictionAir;
    for (var i = 0; i < b.parts.length; i++) b.parts[i].frictionAir = at.frictionAir;
    b.plugin.kind = 'arrow';
    b.plugin.hp = Infinity;
    b.plugin.label = at.name;
    b.plugin.indestructible = true;
    b.plugin.arrowType = arrowType;
    b.plugin.dmgMul = at.dmgMul;
    b.plugin.maxSpeed = at.maxSpeed;
    if (at.special) {
      b.plugin.special = at.special;
      if (at.special === 'fire') { b.plugin.burnDmg = at.burnDmg; b.plugin.burnTime = at.burnTime; }
      if (at.special === 'split') { b.plugin.splitFrame = at.splitFrame; b.plugin.splitAngle = at.splitAngle; b.plugin.dmgMul = at.dmgMul; }
      if (at.special === 'pierce') { b.plugin.pierceCount = at.pierceCount; }
    }
    return b;
  }
  ...
  function newState(levelIdx) {
    ...
    return {
      ...
      arrowQueue: (lv.arrowLoad || ['plain']).slice(),
      arrowType: 'plain',
      subArrows: [],
      burningBodies: [],
      arrowsLeft: lv.arrowLoad ? lv.arrowLoad.length : (lv.arrows || D.ARROWS_PER_LEVEL),
      arrowsTotal: lv.arrowLoad ? lv.arrowLoad.length : (lv.arrows || D.ARROWS_PER_LEVEL),
      birdsTotal: lv.birds.length,
      sunsTotal: lv.suns.length,
      birdsKilled: 0, sunsKilled: 0, blocksKilled: 0,
      comboKills: 0, maxCombo: 0, maxDistance: 0,
      fireKills: 0, pierceKills: 0,
      phase: 'ready',
  ...
  function buildLevel(levelIdx) {
    ...
    for (i = 0; i < lv.blocks.length; i++) {
      addBlock(makeFusan(lv.blocks[i].shape, lv.blocks[i].x, lv.blocks[i].y, lv.blocks[i].wallType));
    }
    ...
    for (i = 0; i < lv.birds.length; i++) {
      var bd = lv.birds[i];
      var bird = makeJinwu(bd.x, bd.perchY - FOOT, { spare: !!bd.spare, type: bd.type, patrolCenter: bd.patrolCenter, patrolRange: bd.patrolRange });
  ...
  function loadArrow() {
    ...
    var type = S.arrowQueue.length > 0 ? S.arrowQueue.shift() : 'plain';
    S.arrowType = type;
    S.arrow = makeArrow(type);
    stampBox(S.arrow);
    M.Body.setPosition(S.arrow, { x: ANCHOR.x, y: ANCHOR.y });
    M.Body.setStatic(S.arrow, true);
  ...
  function launch(vx, vy, fromX, fromY) {
    ...
    if (S.mode === 'level' && S.arrowsLeft <= 0) return false;
    var fx = (fromX === undefined) ? ANCHOR.x : fromX;
    var fy = (fromY === undefined) ? ANCHOR.y : fromY;
    var at = S.arrow.plugin;
    ...
    var ms = at.maxSpeed || MAX_SPEED;
    M.Body.setSpeed(S.arrow, Math.min(M.Body.getSpeed(S.arrow), ms));
  ...
  function retireArrow() {
    ...
    for (var i = 0; i < S.subArrows.length; i++) {
      if (S.subArrows[i]) {
        spawnPuff(S.subArrows[i].position.x, S.subArrows[i].position.y, 4, C.arrowShaft);
        M.Composite.remove(world, S.subArrows[i]);
      }
    }
    S.subArrows = [];
  ...
  M.Events.on(engine, 'collisionStart', function (ev) {
    ...
    for (var i = 0; i < ev.pairs.length; i++) {
      ...
      var impact = M.Vector.magnitude(rel);
      ...
      if (a.plugin.kind === 'arrow' && a.plugin.special === 'pierce' && a.plugin.pierceCount > 0 && !isGroundOrWall(b)) {
        if (a.plugin.preVel) {
          M.Body.setVelocity(a, a.plugin.preVel);
          M.Body.setAngle(a, Math.atan2(a.plugin.preVel.y, a.plugin.preVel.x));
        }
        a.plugin.pierceCount--;
        a.plugin.dmgMul = 0.5;
      }
      if (b.plugin.kind === 'arrow' && b.plugin.special === 'pierce' && b.plugin.pierceCount > 0 && !isGroundOrWall(a)) {
        if (b.plugin.preVel) {
          M.Body.setVelocity(b, b.plugin.preVel);
          M.Body.setAngle(b, Math.atan2(b.plugin.preVel.y, b.plugin.preVel.x));
        }
        b.plugin.pierceCount--;
        b.plugin.dmgMul = 0.5;
      }
      ...
      if (a.plugin.kind === 'arrow' && a.plugin.special === 'fire' && b.plugin.hp !== undefined && !b.plugin.fireResist && !isGroundOrWall(b)) {
        b.plugin.burning = (b.plugin.burning || 0) + (a.plugin.burnTime || 300);
        b.plugin.burnDmg = a.plugin.burnDmg || 3;
      }
      if (b.plugin.kind === 'arrow' && b.plugin.special === 'fire' && a.plugin.hp !== undefined && !a.plugin.fireResist && !isGroundOrWall(a)) {
        a.plugin.burning = (a.plugin.burning || 0) + (b.plugin.burnTime || 300);
        a.plugin.burnDmg = b.plugin.burnDmg || 3;
      }
  ...
  function damage(victim, attacker, impact, px, py) {
    ...
    var dmgMul = (attacker.plugin && attacker.plugin.dmgMul) || 1.0;
    var dmg = impact * DAMAGE_SCALE * dmgMul;
    p.hp -= dmg;
    S.hitLog.push({
  ...
  function destroy(body, px, py) {
    ...
    if (p.deathExplosion) {
      var ex = p.deathExplosion;
      var ex2 = { x: px || body.position.x, y: py || body.position.y };
      S.rings.push({ x: ex2.x, y: ex2.y, t: 0, r: ex.r * 2, big: true });
      spawnPuff(ex2.x, ex2.y, 20, C.jinwuFlame);
      var allTargets = S.birds.concat(S.blocks, S.suns);
      for (var ti = 0; ti < allTargets.length; ti++) {
        var t = allTargets[ti];
        if (t === body || t.plugin.dead) continue;
        var td = Math.hypot(t.position.x - ex2.x, t.position.y - ex2.y);
        if (td <= ex.r) {
          t.plugin.hp -= ex.dmg;
          S.hitLog.push({ kind: t.plugin.kind, label: t.plugin.label, impact: 0, dmg: ex.dmg, hpAfter: +Math.max(0, t.plugin.hp).toFixed(1), killed: t.plugin.hp <= 0, by: p.label + '爆炸', at: { x: Math.round(ex2.x), y: Math.round(ex2.y) }, mode: S.mode, frame: S.frameCount, arrowFrame: S.arrowInFlight ? S.flightFrames : -1 });
          if (t.plugin.hp <= 0) destroy(t, t.position.x, t.position.y);
        }
      }
    }
    ...
    if (p.shardOnBreak) {
      ...
      for (var si = 0; si < 5; si++) {
        var ang = (si / 5) * Math.PI * 2;
        var shard = M.Bodies.rectangle(body.position.x, body.position.y, sz, sz, {
          density: 0.003, friction: 0.5, restitution: 0.3
        });
        shard.plugin = { kind: 'shard', hp: 1, label: '碎片', indestructible: false };
        M.Body.setVelocity(shard, { x: Math.cos(ang) * 3, y: Math.sin(ang) * 3 - 2 });
        M.Composite.add(world, shard);
      }
      spawnPuff(body.position.x, body.position.y, 12, '#B0E0E6');
    }

    var i;
    if ((i = S.birds.indexOf(body)) >= 0) {
      S.birds.splice(i, 1); S.birdsKilled++;
      S.comboKills++;
      if (S.comboKills > S.maxCombo) S.maxCombo = S.comboKills;
      if (S.launchX !== undefined) {
        var d = Math.hypot(body.position.x - S.launchX, body.position.y - S.launchY);
        if (d > S.maxDistance) S.maxDistance = d;
      }
      var arrow = S.arrow;
      if (arrow && arrow.plugin.special === 'fire') S.fireKills++;
      if (arrow && arrow.plugin.special === 'pierce') S.pierceKills++;
    }
  ...
  function buildResult(win, reason) {
    ...
    var baseScore = kills * 120 + S.sunsKilled * 25 + S.blocksKilled * 15;
    var efficiencyBonus = Math.max(0, S.arrowsLeft) * 40;
    var comboBonus = S.maxCombo >= 2 ? (S.maxCombo - 1) * 50 : 0;
    var distanceBonus = S.maxDistance > 600 ? 30 : 0;
    var perfectBonus = (win && used === 0) ? 0 : (win && S.arrowsLeft >= 1) ? 0 : 0;
    if (win && S.arrowsLeft === S.arrowsTotal) perfectBonus = 100;
    var score = baseScore + efficiencyBonus + comboBonus + distanceBonus + perfectBonus;
    ...
    return {
      ...
      baseScore: baseScore, efficiencyBonus: efficiencyBonus,
      comboBonus: comboBonus, distanceBonus: distanceBonus, perfectBonus: perfectBonus,
      maxCombo: S.maxCombo, maxDistance: Math.round(S.maxDistance),
      fireKills: S.fireKills, pierceKills: S.pierceKills
    };
  }
  ...
  function checkEnd() {
    ...
    if (S.goal === 'spare') {
      ...
      if (S.suns.length === 0 && !S.arrowInFlight) { S.phase = 'won'; S.result = buildResult(true, 'clear'); }
      return;
    }
    if (S.birds.length === 0 && !S.arrowInFlight) { S.phase = 'won'; S.result = buildResult(true, 'clear'); }
  }
  ...
  function setNock(p) {
    ...
    var v = launchVector();
    S.previewPts = v.power < 1.2 ? [] : simulate(ANCHOR.x, ANCHOR.y, v.vx, v.vy, 110);
    S.phase = 'dragging';
  ...
  function inVisualShape(b, x, y) {
    if (!b.circleRadius || b.plugin.kind !== 'jinwu') return inSilhouette(b, x, y);
    var shape = D.SHAPES.jinwu, names = Object.keys(shape);
    var bx = b.position.x, by = b.position.y, ang = b.angle;
    var cos = Math.cos(-ang), sin = Math.sin(-ang), s = D.BIRD_SCALE;
    var dx = x - bx, dy = y - by;
    var lx = (dx * cos - dy * sin) / s, ly = (dx * sin + dy * cos) / s;
    for (var i = 0; i < names.length; i++) {
      var verts = shape[names[i]];
      var inside = false;
      for (var j = 0, k = verts.length - 1; j < verts.length; k = j++) {
        var xi = verts[j][0], yi = verts[j][1], xj = verts[k][0], yj = verts[k][1];
  ...
  function solveAim(tx, ty, requireMiss, targetBody) {
    ...
    var nx = ANCHOR.x, ny = ANCHOR.y;
    for (ai = 0; ai <= 176; ai++) {
      ang = (-88 + ai * 1.0) * Math.PI / 180;
      var maxPw = (S && S.arrow && S.arrow.plugin.maxSpeed) || MAX_PULL * POWER_SCALE;
    for (pw = 6; pw <= maxPw + 0.001; pw += 0.5) {
  ...
  function checkArrowMisses(pts, b) {
    ...
    var r = b.circleRadius ? b.circleRadius + 6 : 0;
    for (s = 0; s < pts.length; s++) {
      ...
      for (q = 0; q < offsets.length; q++) {
        ...
        if (r && Math.hypot(sx - b.position.x, sy - b.position.y) <= r) return { ok: false, inBox: inBox };
      }
    }
  ...
  function rangeShot(kind) {
    ...
    var sol, gp;
    if (kind === 'ghost') {
      var cands = findGhostPoints(S.rangeBird, 10);
      if (!cands.length) return null;
  ...
  function stepOnce() {
    ...
    if (S.arrow && S.arrow.plugin.special === 'pierce' && S.arrow.plugin.pierceCount > 0) {
      S.arrow.plugin.preVel = { x: S.arrow.velocity.x, y: S.arrow.velocity.y };
    }
    ...
    if (S.birds) {
      for (var bi = S.birds.length - 1; bi >= 0; bi--) {
        var bird = S.birds[bi];
        var bp = bird.plugin;
        if (bp.burning && bp.burning > 0) {
          bp.burning--;
          if (S.frameCount % 60 === 0) {
            bp.hp -= bp.burnDmg || 3;
            spawnPuff(bird.position.x, bird.position.y, 3, C.jinwuFlame);
            if (bp.hp <= 0) { destroy(bird, bird.position.x, bird.position.y); continue; }
          }
        }
        if (bp.patrolCenter !== undefined && !bird.isStatic && S.frameCount % 2 === 0) {
          bp.patrolPhase = (bp.patrolPhase || 0) + 0.04;
          var targetX = bp.patrolCenter + Math.sin(bp.patrolPhase) * (bp.patrolRange || 80);
          M.Body.setPosition(bird, { x: targetX, y: bird.position.y });
          M.Body.setVelocity(bird, { x: 0, y: 0 });
        }
      }
    }
    if (S.blocks) {
      for (var bli = S.blocks.length - 1; bli >= 0; bli--) {
        var blk = S.blocks[bli];
        var blp = blk.plugin;
        if (blp.burning && blp.burning > 0) {
          blp.burning--;
          if (S.frameCount % 60 === 0) {
            blp.hp -= blp.burnDmg || 3;
            spawnPuff(blk.position.x, blk.position.y, 3, C.jinwuFlame);
            if (blp.hp <= 0) { destroy(blk, blk.position.x, blk.position.y); continue; }
          }
        }
      }
    }
    if (S.suns) {
      for (var si2 = S.suns.length - 1; si2 >= 0; si2--) {
        var sn = S.suns[si2];
        var snp = sn.plugin;
        if (snp.burning && snp.burning > 0) {
          snp.burning--;
          if (S.frameCount % 60 === 0) {
            snp.hp -= snp.burnDmg || 3;
            spawnPuff(sn.position.x, sn.position.y, 3, C.jinwuFlame);
            if (snp.hp <= 0) { destroy(sn, sn.position.x, sn.position.y); continue; }
          }
        }
      }
    }
    ...
    if (S.arrowInFlight && S.arrow) {
      ...
      var ms = S.arrow.plugin.maxSpeed || MAX_SPEED;
      if (sp > ms) M.Body.setSpeed(S.arrow, ms);
      if (sp > 1) {
        M.Body.setAngle(S.arrow, Math.atan2(S.arrow.velocity.y, S.arrow.velocity.x));
        M.Sleeping.set(S.arrow, false);
      }
    ...
    if (S.arrowInFlight && S.arrow && S.arrow.plugin.special === 'split' && S.arrow.plugin.splitFrame > 0) {
      S.arrow.plugin.splitFrame--;
      if (S.arrow.plugin.splitFrame === 0) {
        S.arrow.plugin.splitFrame = -1;
        var av = S.arrow.velocity;
        var aang = Math.atan2(av.y, av.x);
        var asp = Math.hypot(av.x, av.y);
        for (var ss = -1; ss <= 1; ss += 2) {
          var subAng = aang + ss * S.arrow.plugin.splitAngle;
          var sub = makeArrow('split');
          sub.plugin.splitFrame = -1;
          sub.plugin.dmgMul = 0.6;
          M.Body.setStatic(sub, false);
          M.Body.setPosition(sub, S.arrow.position);
          M.Body.setAngle(sub, subAng);
          M.Body.setVelocity(sub, { x: Math.cos(subAng) * asp, y: Math.sin(subAng) * asp });
          M.Composite.add(world, sub);
          S.subArrows.push(sub);
        }
      }
    }
    ...
    all.forEach(function (b) {
      ...
      for (var x = Math.ceil(bb.min.x); x <= Math.floor(bb.max.x); x += step) {
        for (var y = Math.ceil(bb.min.y); y <= Math.floor(bb.max.y); y += step) {
          ...
          var inside = inVisualShape(b, cx, cy);
          var other = false;
          if (!inside) {
            for (var k = 0; k < all.length; k++) {
              if (all[k] === b) continue;
              if (inVisualShape(all[k], cx, cy)) { other = true; break; }
            }
          }
          out.push({
            kind: b.plugin.kind, shape: b.plugin.shapeName || 'circle',
  ...
  window.HYEngine = {
    ...
    drag: function (px, py) {
      ...
      if ((S.phase !== 'ready' && S.phase !== 'dragging') || S.arrowInFlight || !S.arrow) return 0;
      S.dragActive = true;
