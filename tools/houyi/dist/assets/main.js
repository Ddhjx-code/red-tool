...
(function () {
  ...
  function loadSave() {
    try {
      ...
      return {
        ...
        achievements: o.achievements instanceof Array ? o.achievements : [],
        stats: (o.stats && typeof o.stats === 'object') ? o.stats : {}
      };
    } catch (e) { return { cleared: [], best: {}, sunsDown: 0, achievements: [], stats: {} }; }
  }
  ...
  function recordWin(res) {
    ...
    var st = o.stats;
    st.totalArrows = (st.totalArrows || 0) + res.arrowsUsed;
    st.totalKills = (st.totalKills || 0) + (res.birdsKilled || 0) + (res.sunsKilled || 0);
    st.totalBlocks = (st.totalBlocks || 0) + (res.blocksKilled || 0);
    st.maxCombo = Math.max(st.maxCombo || 0, res.maxCombo || 0);
    st.maxDistance = Math.max(st.maxDistance || 0, res.maxDistance || 0);
    st.fireKills = (st.fireKills || 0) + (res.fireKills || 0);
    st.pierceKills = (st.pierceKills || 0) + (res.pierceKills || 0);
    st.clearedCount = (st.clearedCount || 0) + 1;

    o.stats = st;

    var ach = D.ACHIEVEMENTS;
    function has(id) { return o.achievements.indexOf(id) >= 0; }
    function unlock(id) { if (!has(id)) { o.achievements.push(id); toast('成就解锁：' + ach[id].name, 'ok'); } }

    if (st.clearedCount >= 1) unlock('first_shot');
    if (o.sunsDown >= 9) unlock('nine_down');
    if (res.goal === 'spare' && res.win) unlock('one_sun');
    if (res.maxDistance >= 800) unlock('long_shot');
    if (res.maxCombo >= 2) unlock('chain_kill');
    if (res.win && res.arrowsLeft >= 3) unlock('efficiency');
    if (st.fireKills >= 5) unlock('fire_master');
    if (st.totalBlocks >= 50) unlock('destroyer');
    if (st.pierceKills >= 2) unlock('pierce_kill');
    var allThree = D.LEVELS.every(function (lv) { return o.best[lv.id] && o.best[lv.id].stars >= 3; });
    if (allThree) unlock('all_stars');
    var others = Object.keys(ach).filter(function (k) { return k !== 'grandmaster'; });
    if (others.every(function (k) { return has(k); })) unlock('grandmaster');
  ...
  function showHome() {
    ...
    var achEl = $('home-achievements');
    if (achEl) achEl.textContent = '成就 ' + o.achievements.length + ' / ' + Object.keys(D.ACHIEVEMENTS).length;
  ...
  function showAchievements() {
    ...
    if (!box) return;
    box.innerHTML = '';
    Object.keys(D.ACHIEVEMENTS).forEach(function (id) {
      var a = D.ACHIEVEMENTS[id];
      var unlocked = o.achievements.indexOf(id) >= 0;
      var row = document.createElement('div');
      row.className = 'ach-card' + (unlocked ? '' : ' locked');
      var nm = document.createElement('b');
      nm.textContent = a.name;
      var ds = document.createElement('p');
      ds.textContent = a.desc;
      var st = document.createElement('i');
      st.textContent = unlocked ? '已解锁' : '未解锁';
      row.appendChild(nm); row.appendChild(ds); row.appendChild(st);
      box.appendChild(row);
    });
    $('ach-count').textContent = o.achievements.length + ' / ' + Object.keys(D.ACHIEVEMENTS).length;
    setView('view-achievements');
  }
  ...
  function showIntro(idx) {
    ...
    var atBox = $('intro-arrows');
    if (atBox) {
      atBox.innerHTML = '';
      var typeNames = { plain: '素缯', fire: '火箭', heavy: '重箭', split: '裂箭', pierce: '穿云' };
      var typeDesc = {
        plain: '素缯白箭，无特殊效果',
        fire: '命中后燃烧目标，持续造成伤害',
        heavy: '密度高、抗风阻，冲击力强',
      ...
      var counts = {};
      (lv.arrowLoad || ['plain']).forEach(function (t) { counts[t] = (counts[t] || 0) + 1; });
      Object.keys(counts).forEach(function (t) {
        var p = document.createElement('p');
        p.textContent = typeNames[t] + ' ×' + counts[t] + ' — ' + typeDesc[t];
        atBox.appendChild(p);
      });
      atBox.style.display = '';
    }
  ...
  function updateHud() {
    ...
    var at = S.arrowType || 'plain';
    var typeNames = { plain: '素缯', fire: '火箭', heavy: '重箭', split: '裂箭', pierce: '穿云' };
    var typeEl = $('hud-arrow-type');
    if (typeEl) typeEl.textContent = typeNames[at] || at;
  }
  ...
  function showResult() {
    ...
    if (bd && win) {
      var lines = [];
      lines.push('基础 ' + res.baseScore);
      if (res.efficiencyBonus) lines.push('余箭 +' + res.efficiencyBonus);
      if (res.comboBonus) lines.push('连击 +' + res.comboBonus);
      if (res.distanceBonus) lines.push('远射 +' + res.distanceBonus);
      if (res.perfectBonus) lines.push('完美 +' + res.perfectBonus);
      if (res.maxCombo >= 2) lines.push('最高连击 ' + res.maxCombo);
      if (res.maxDistance > 0) lines.push('最远射程 ' + Math.round(res.maxDistance));
      bd.textContent = lines.join(' · ');
      bd.style.display = lines.length > 1 ? '' : 'none';
    }
  ...
  function init() {
    ...
    if (btnAch) btnAch.addEventListener('click', function () { showAchievements(); });
    var btnAchBack = $('btn-ach-back');
    if (btnAchBack) btnAchBack.addEventListener('click', function () { showHome(); });
