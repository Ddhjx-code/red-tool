...
| 结局揭示 | 月亮盈亏一次渐变 ≤ 1.2 秒 | 唯一允许的长动效，`transition: opacity 1.2s` 一次性 |
...
10. **从容模式：§4.2 与 §4.3.3 不矛盾，二者分别管「计时」与「摆动手感」，实现两侧都遵守。**
    §4.2 移除的是**倒计时上限**（8/10/12/10 秒预算）；§4.3.3（行 394/402）明文从容模式下
    **摆动速率不变**（单程 2.0s、驻留 0.40s）、只把「4 次/5 次过窗」变成「无限次过窗」，
    **手感难度与正常模式完全相同**。摆杆是输入机制（S1 力度条 / S4 火候条），不是计时器，
    故从容模式下摆杆照常摆动，玩家仍需在 0.40s 驻留窗口内点「停」——只是不再有次数/时间上限。
    实现：`scene.js` 的 `paintStep` 对 S1/S4 条**无条件加 `is-swinging`**（摆杆常驻），
    计时器 `setTimeout` 仅在 `!congRong` 时挂载（§4.2 移除计时）；从容模式下摆杆无限摆动、
    无自动确认，玩家点「停」锁定 offset。精度公式与 `E.precision`/`E.tolOf` 完全不变。
    （早前 Task 11 曾误读为「从容模式关掉摆杆、静态可拖」，那会让 S1/S4 拖到中心即满分、
    难度归零，违背 §4.2「让步只给时间不给分数」与 §4.3.3「手感难度完全相同」，已按 TDD 修正。）

    回归断言：`tests/yueyan_craft.py::test_cong_rong`
    （从容模式**摆杆常驻** `is-swinging`、无计时器自动确认、品级阈值不变、仍可逐步出饼）
    与 `test_soft_timer`（普通模式 4 步预算 8/10/12/10 秒，`D.steps` 单一来源）。
...
16. **§8.3 全部质感由四个纯程序化函数产生，仓库零图片资产。**
    ...
    DOM 内 canvas 仍只有 `#end-moon` 一个（§8.3 不逐帧重绘）。
...
| 04-preview | `preview-seats` 5、`preview-banquet` 5、`preview-lanterns` 4；
  ...
  五个宴备槽位标签均非空；到席座位带称谓。 |
| 05-ending | `end-moon` canvas 非空且含月色 `rgb(242,228,196)`（#F2E4C4）像素；
  canvas `data-ending` = E1；`end-name` 非空；`end-text` 与
  `D.endings.E1.text` 逐字相等；name/text 内无任何数字（不显示分数品级）。 |
