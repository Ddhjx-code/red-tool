...
网关对 `parameters.size` 的**总像素数**有硬性区间，超出即 HTTP 400 而非静默回落：

```
InvalidParameter: Total pixels (262144) must be between 589824 and 16777216.
```

| 请求尺寸 | 总像素 | 结果 |
| --- | --- | --- |
| `512*512` | 262,144 | **HTTP 400 拒绝**（低于下限） |
| `768*768` | 589,824 | **HTTP 200 接受**，返回 768×768 RGBA |
| `720*1280` | 921,600 | HTTP 200 接受（此前已验证） |

- 下限 589,824 = 768²，且**边界含端点**（768×768 实测通过）。
- 上限 16,777,216 = 4096²。
- 返回图像 `mode=RGBA`，自带 alpha 通道，可直接用于抠图素材。
- 768×768 PNG 实测 856 KB，必须转 webp 后入库。

**推论：任何小于 768×768 的目标素材都无法直接生成。** 小尺寸素材（512 纹理 /
256 灯笼 / 128 图标）必须先在 ≥ 768×768 生成，再用 Pillow 本地降采样到目标尺寸。
降采样是**强制步骤**，不是可选回退。附带收益是降采样后 webp 体积显著小于
720×1280 的实测值（q85 = 175 KB）。

（探针为临时脚本，未入仓库；约束已记录于本文）
